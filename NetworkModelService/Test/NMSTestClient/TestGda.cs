     ...
     public class TestGda : IDisposable
     {
          ...
          public List<long> GetRelatedValues(long sourceGlobalId, List<ModelCode> properties, Association association)
          {
               ...
               try
               {
                    ...
                    //var path = Config.Instance.ResultDirecotry + "\\GetRelatedValues_Results.xml";
                    var path = Path.GetFullPath(Path.Combine(Directory.GetCurrentDirectory(), @"..\..\..\Results\GetRelatedValues_Results.xml"));
                    xmlWriter = new XmlTextWriter(path, Encoding.Unicode);
                    xmlWriter.Formatting = Formatting.Indented;
